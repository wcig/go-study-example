...
func saveMsg(msg *message) {
	...
	// 判断该用户该类型消息是否存在
	n, _ := client.Exists(msgContentKey).Result()
	if exist := n == 1; !exist {
		// 消息不存在
		// 保存消息内容为string类型
		bytes, _ := json.Marshal(msg)
		client.Set(msgContentKey, string(bytes), 2*msgAggDuration)

		// 添加标志位到推送消息列表
		itemVal := fmt.Sprintf(msgPushListItemFormat, msg.UserId, msg.Type, time.Now().Unix())
		client.RPush(msgPushFlagListKey, itemVal)
	}
}
...
func pollMsg() {
	for {
		itemVal, _ := client.LPop(msgPushFlagListKey).Result()
		...
		if time.Now().Unix() >= ts+msgAggDuration {
			...
			msgVal, _ := client.Get(msgContentKey).Result()

			var msg message
			json.Unmarshal([]byte(msgVal), &msg)
			pushMsg(msg)

			client.Del(msgContentKey)
		} else {
			// 聚合时间未到,重新添加到list中
			client.LPush(msgPushFlagListKey, itemVal)
