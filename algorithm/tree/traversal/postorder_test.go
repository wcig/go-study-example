...
// 后序遍历 (辅助栈实现-统一迭代法)
func TestPostorderTraversalWithStack2(t *testing.T) {
	root := getRoot()
	var s []int
	postorderWithStack2(root, &s)
	fmt.Println(">> postorderWithStack2:", s) // >> postorderWithStack2: [4 5 2 6 7 3 1]
}

func postorderWithStack2(root *TreeNode, s *[]int) {
	if root == nil {
		return
	}
	stack := []*TreeNode{root}
